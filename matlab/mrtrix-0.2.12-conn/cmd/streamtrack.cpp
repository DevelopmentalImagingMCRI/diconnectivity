...
class Threader {
  ...
  protected:
    ...
    void append (std::vector<Point>*& tck, bool accept, int firstendregion, int secondendregion, Point seed)
    {
      mutex.lock();
      if (accept) {
        fifo.push (tck);
        tck = NULL;
		std::cout << firstendregion << " " << secondendregion << " " << seed[0] << " " << seed[1] << " " << seed[2] << std::endl;
        data_ready.signal();
      }
    ...
    void execute (Tracker::Base* tracker) 
    {
      ...
      while (writer.count < max_num_tracks && ( max_num_attempts ? writer.total_count < max_num_attempts : true )) {
        ...
        if (!tracker->track_excluded() && !unidirectional) {
		  ...
		  tracker->start_second_track();
          while (tracker->next()) tck->push_back (tracker->position());
        }

        append (tck, (!tracker->track_excluded() && tracker->track_included() && tck->size() > min_size), tracker->get_firstendregion(), tracker->get_secondendregion() , seed);
      }
