				...
				class Base {
					public:
						...
						{ 
							...
							//	if (!i->included) return (false);
							return (firstendregion > 0) && (secondendregion > 0);
							//turn (true);
						}
						
						void start_second_track () { doingfirst = false; entered_inclusion = false; }
						int get_firstendregion () { return firstendregion; }
						int get_secondendregion () { return secondendregion; }
						
						//void save_seed () { track_seed = pos; }
						...
						class Mask {
							public:
								...
								int nearest_int_in_mask (const Point& pt) {
									if (!pt.valid()) return false;
									Point y (i.R2P (pt));
									if (y[0] < lower[0] || y[0] >= upper[0] || y[1] < lower[1] || y[1] >= upper[1] || y[2] < lower[2] || y[2] >= upper[2]) return (0);

									i.set (0, int(y[0]+0.5));
									i.set (1, int(y[1]+0.5));
									i.set (2, int(y[2]+0.5));
									return ((int)i.Image::Position::value());
								}
					...
					protected:
						...
						int firstendregion, secondendregion;
						bool doingfirst;
