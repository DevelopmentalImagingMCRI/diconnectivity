function [varargout] = mrtrix_create_tracks_wb_wm_adhd(Subject, MrtrixMethod, SeedType, Curvature, ROIsToSaveStreamlines)
...
if nargin > 4
	if(iscellstr(ROIsToSaveStreamlines))
		[~, LOC] = ismember(ROIsToSaveStreamlines, CurLabels.shortlabels);
		if(any(LOC == 0))
			error('Region not found in labels');
		end
		ROIsToSaveStreamlines = LOC;
		clear LOC;
	else
		if(any(~ismember(ROIsToSaveStreamlines, 1:length(CurLabels.shortlabels))))
			error('The indices of the ROIsToSaveStreamlines are invalid');
		end
	end
else
	ROIsToSaveStreamlines = [];
end

WMMaskNII = load_nii(fullfile(Subject, 'freesurfer_wm'));
WMMaskIMG = flipdim(permute(WMMaskNII.img, [2 1 3]), 1);

% this creates too many tracks for high res volumesssh-keygen -f "/home/addo/.ssh/known_hosts" -R i9
...
%	keyboard;
	CurNumTracks = sum(AcceptedTrackMask);	
	NumBeforeIpsi = sum(AcceptedTrackMask);
	IDX = find(AcceptedTrackMask);
	RejectedIpsiInterStreamlines = false(CurNumTracks, 1);
	for z = 1:BlockSize:CurNumTracks
		RightIDX = min(z + BlockSize - 1, CurNumTracks);
		CurIDX = z:RightIDX;
		
		RejectedIpsiInterStreamlines(CurIDX) = mrtrix_create_tracks_ipsi_rois_crossing(CurTracks(IDX(CurIDX)), OriginalSeedIMG, [StartRegion(IDX(CurIDX)) EndRegion(IDX(CurIDX))], CurLabels.shortlabels);
	end
	AcceptedTrackMask(AcceptedTrackMask) = ~RejectedIpsiInterStreamlines;

	clear RejectedAnteriorCCPosterior;
	disp([num2str(NumBeforeIpsi - sum(AcceptedTrackMask)) ' Ipsi']);
...
% 	S = TracksToAdd(1);%tracks_world_to_img(T.Tracks, WMMaskNII, size(WMMaskIMG));
% 	clf;
% 	H1 = isosurface(SeedIMG == ToAddStartRegions(1), 0.5);
% 	H2 = isosurface(SeedIMG == ToAddEndRegions(1), 0.5);
% 	patch(H1, 'FaceColor', 'r');
% 	hold on;
% 	patch(H2, 'FaceColor', 'b');
% 
% 	streamline(S);
%	keyboard;%
	%keyboard;
...
% modification, save
if(~isempty(ROIsToSaveStreamlines))
	TracksDirName = ['tracks_' SeedType '_' MrtrixMethod '_accepted_tracks_' num2str(Curvature) '_roipairs'];
	try
		rmdir(fullfile(Subject, TracksDirName), 's');
	catch e
		disp('already removed');
	end
	[~,~,~] = mkdir(fullfile(Subject, TracksDirName));
	
	disp('Saving tracks, pairs of ROIs');
	for I = 1:length(ROIsToSaveStreamlines)
		for J = I + 1:length(ROIsToSaveStreamlines)
			
			T = EmptyTracksHeader;
			T.Tracks = FinalTracks(...
				(FinalStartRegions == ROIsToSaveStreamlines(I) | FinalEndRegions == ROIsToSaveStreamlines(I)) & ...
				(FinalStartRegions == ROIsToSaveStreamlines(J) | FinalEndRegions == ROIsToSaveStreamlines(J)));

			if(~isempty(T.Tracks))
				
				disp([CurLabels.shortlabels{ROIsToSaveStreamlines(I)} '-' CurLabels.shortlabels{ROIsToSaveStreamlines(J)}]);
% 				S = tracks_world_to_img(T.Tracks, WMMaskNII, size(WMMaskIMG));
% 				clf;
% 				H1 = isosurface(SeedIMG == ROIsToSaveStreamlines(I), 0.5);
% 				H2 = isosurface(SeedIMG == ROIsToSaveStreamlines(J), 0.5);
% 				patch(H1, 'FaceColor', 'r');
% 				hold on;
% 				patch(H2, 'FaceColor', 'b');
% 
% 				streamline(S);
% 				keyboard;
				F = fullfile(Subject, TracksDirName, [CurLabels.shortlabels{ROIsToSaveStreamlines(I)} '-' CurLabels.shortlabels{ROIsToSaveStreamlines(J)} '.tck']);
				save_mif_tracks(T, F);
				F = fullfile(Subject, TracksDirName, [CurLabels.shortlabels{ROIsToSaveStreamlines(I)} '-' CurLabels.shortlabels{ROIsToSaveStreamlines(J)} '.trk']);
